...
describe SlackMoji::Commands::Subscription, vcr: { cassette_name: 'slack/user_info' } do
  ...
  context 'team' do
    ...
    it 'is a subscription feature' do
      expect(message: "#{SlackRubyBot.config.user} subscription", user: 'user').to respond_with_slack_message(
        "Your trial subscription has expired. Subscribe your team for $9.99 a year at #{SlackMoji::Service.url}/subscribe?team_id=#{team.team_id} to continue having emojis in your life."
      )
    end
  end
  context 'team without a customer ID' do
    ...
    it 'errors' do
      expect(message: "#{SlackRubyBot.config.user} subscription", user: 'user').to respond_with_slack_message(
        "Not a subscriber. Subscribe your team for $9.99 a year at #{SlackMoji::Service.url}/subscribe?team_id=#{team.team_id} to continue having emojis in your life."
      )
    end
  end
  shared_examples_for 'subscription' do
    ...
    context 'with a plan' do
      before do
        stripe_helper.create_plan(id: 'moji-yearly', amount: 999)
      end
